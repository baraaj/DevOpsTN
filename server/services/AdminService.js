...
adminService.ajouter = (admin) => {
    return new Promise((reslove,reject) => {
        var {nom,prenom,email,password,cni,datembauche}=admin;
        db.query(`INSERT INTO ADMIN VALUES('${nom}','${prenom}','${email}','${password}','${cni}','${datembauche}')`,(err,result) => {
            let res = {};
            if(err) res.error = err;
            res.message = `ajout D'etudiant ${nom} reussit`;
            reslove(res);
        });
    })
}
...
adminService.supprimer = (admin) => {
    return new Promise((reslove,reject) => {
        ...
        db.query(`DELETE FROM ADMIN where id=${id}`,(err,result) => {
            let res = {};
            if(err) res.error = err;
            res.message = `supression D'etudiant ${nom} reussite`;
            reslove(res);
        });
    });
}
